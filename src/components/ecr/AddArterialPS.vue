<template>
    <div>
            ...
            <BCard no-body id="applicationList">
                <BCardHeader class="border-0">
                    <div class="d-md-flex align-items-center">
                        <h5 class="card-title mb-3 mb-md-0 flex-grow-1">Артериальное давление</h5>
                            ...
                            <table class="table table-nowrap align-middle" id="jobListTable">
                                <thead class="text-muted table-light">
                                    <tr class="text-uppercase">
                                        ...
                                        <th class="small-font sort"@click="sortBy('creation_date', 'arterial_pressure_statistic')">
                                            Дата
                                        </th>
                                        <th class="small-font sort" @click="sortBy('systolic_pressure', 'arterial_pressure_statistic')">
                                            Систолическое давление
                                        </th>
                                        <th class="small-font sort" @click="sortBy('diastolic_pressure', 'arterial_pressure_statistic')">
        ...
        <BModal v-model="createAppModal" id="showModal" title="Артериальное Давление" header-class="bg-light p-3"
                ...
                <BCardBody class="border  border-0">
                    <BForm @submit.prevent="addTemp">
                        <BRow class="g-3">
                            <BCol sm="12">
                                <div class="mb-3">
                                    <label class="form-label">Сиастолическое давление </label>
                                    <input v-model="form_temp.systolic_pressure" type="number" :class="{'is-invalid': v$.systolic_pressure.$error }"
                                        placeholder="Сиастолическое давление" class="form-control" />
                                    <span v-if="v$.systolic_pressure.$error" class="text-danger">Это обязательное поле</span>
                                </div>
                                <div class="my-3">
                                    <label class="form-label">Диастолическое давление </label>
                                    <input v-model="form_temp.diastolic_pressure" type="number" :class="{'is-invalid': v$.diastolic_pressure.$error }"
                                        placeholder="Диастолическое давление" class="form-control" />
                                        <span v-if="v$.systolic_pressure.$error" class="text-danger">Это обязательное поле</span>
                                </div>
                                <div class="my-3">
...
const temp_list = computed(() => {
    return store.state.charts.arterial_pressure_statistic.filter(item => item.systolic_pressure !== null && item.diastolic_pressure !== null);
});
...
const filteredTempList = computed(() => {
  ...
  if (searchTerm.value.trim() !== '') {
    return tempList.filter(item => {
      return (
        (item.systolic_pressure !== null && item.systolic_pressure.toString().includes(searchTerm.value.trim())) ||
        (item.creation_date !== null && item.creation_date.includes(searchTerm.value.trim())) ||
        (item.diastolic_pressure !== null && item.diastolic_pressure.toString().includes(searchTerm.value.trim()))
      );
    });
  }
...
const addTemp = async () => {
    ...
    try {
        const response = await axiosInstance.post(`/health-diary-records/add_arterial_pressure/?by_er_card=${route.params.id}`, form_temp.value);
        if (response.status === 200) {
            Swal.fire({
                icon: 'success',
                title: 'Успешно',
                text: 'Аритериальное давление добавлена!',
                timer: 2500,
            ...
            temp_list.value = store.state.charts.arterial_pressure_statistic;
        }
    } catch (error) {
        Swal.fire({
            icon: 'error',
