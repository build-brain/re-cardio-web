<template>
    <div>
            ...
            <BCard no-body id="applicationList">
                <BCardHeader class="border-0">
                    <div class="d-md-flex align-items-center">
                        <h5 class="card-title mb-3 mb-md-0 flex-grow-1">Усталость</h5>
                            ...
                            <table class="table table-nowrap align-middle" id="jobListTable">
                                <thead class="text-muted table-light">
                                    <tr class="text-uppercase">
                                        ...
                                        <th class="small-font sort" @click="sortBy('creation_date', 'fatigue')">
                                            Дата
                                        </th>
                                        <th class="small-font sort" @click="sortBy('fatigue', 'fatigue')">
                                            Усталость
                                        </th>
                                        <th class="small-font sort" @click="sortBy('fatigue_type', 'fatigue')">
                                            Тип Усталости
                                        </th>
                                    </tr>
                                </thead>
                                    ...
                                    <tr v-for="(item, index) in filteredTempList" :key="index">
                                        ...
                                        <td class="designation">
                                            <span class="ps-4">
                                                {{
                                    item.fatigue_type
                                        ? (item.fatigue_type === 'moderate' ? 'умеренная'
                                            : item.fatigue_type === 'quickly_passing' ? 'быстро проходящая'
                                                : item.fatigue_type === 'short_term' ? 'краткосрочная'
                                                    : 'долгосрочная')
                                        : 'не указана'
                                }}</span>
        ...
        <BModal v-model="createAppModal" id="showModal" title="Усталость" header-class="bg-light p-3"
                ...
                <BCardBody class="border  border-0">
                    <BForm @submit.prevent="addTemp">
                        <BRow class="g-3">
                            <BCol sm="12">
                                <div class="mb-3">
                                    <label class="form-label">Усталость</label>
                                    <input v-model="form_temp.fatigue" type="checkbox" placeholder="Усталость"
                                        class="form-check" />

                                </div>
                                <div class="my-3">
                                    <label class="form-label">Тип Усталости </label>
                                    <select class="form-select" v-model="form_temp.fatigue_type" :class="{'is-invalid': v$.fatigue_type.$error}">
                                        <option disabled selected :value="null">Выберите тип</option>
                                        <option v-for="(translation, key) in fatigueTypeTranslations" :key="key"
                                            :value="key">{{ translation }}</option>
                                            
                                    </select>
                                    <span class="text-danger" v-if="v$.fatigue_type.$error">Это поле обязательное.</span>
                                </div>
                                <div class="my-3">
...
const temp_list = computed(() => {
    return store.state.charts.fatigue.filter(item => item.fatigue !== null && item.fatigue_type !== null);
});
...
const filteredTempList = computed(() => {
    ...
    if (searchTermTrimmed !== '') {
        return tempList.filter(item => {
            const dyspneaMatches = item.fatigue !== null && (item.fatigue ? 'да' : 'нет').includes(searchTermTrimmed);
            const creationDateMatches = item.creation_date !== null && item.creation_date.toString().toLowerCase().includes(searchTermTrimmed);
            const dyspneaTypeMatches = item.fatigue_type !== null && fatigueTypeTranslations[item.fatigue_type].toLowerCase().includes(searchTermTrimmed);
...
const addTemp = async () => {
    ...
    try {
        const response = await axiosInstance.post(`/health-diary-records/add_fatigue/?by_er_card=${route.params.id}`, form_temp.value);
        if (response.status === 200) {
            ...
            temp_list.value = store.state.charts.fatigue;
        }
    } catch (error) {
        Swal.fire({
            icon: 'error',
