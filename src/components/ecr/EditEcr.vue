...
const getInstaceCaSheet = async () => {
    const response = await axiosInstance.get(`/ca-sheets/${route.params.id}/`);
    ca_sheets.value = response.data;
    affected_vessel.value = response.data.affected_vessels;
    // Здесь мы инициализируем complication1, complication2 и complication3
    complication1.value = ca_sheets.value.complications.filter(id => formattedComplicationsGroup1.value.some(c => c.value === id));
    complication2.value = ca_sheets.value.complications.filter(id => formattedComplicationsGroup2.value.some(c => c.value === id));
    complication3.value = ca_sheets.value.complications.filter(id => formattedComplicationsGroup3.value.some(c => c.value === id));
};
...
const onHandlesubmit = async () => {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            const currentVessels = response.data.affected_vessels;
            const updatedIds = updatedAffectedVessels.map(vessel => vessel.id);
            const deletedVessels = currentVessels.filter(vessel => !updatedIds.includes(vessel.id));
            for (const vessel of deletedVessels) {
                await axiosInstance.delete(`/affected-vessels/${vessel.id}/`);
            }
            for (const vessel of updatedAffectedVessels) {
                if (vessel.id) {
                    await axiosInstance.put(`/affected-vessels/${vessel.id}/`, vessel);
                } else {
        ...
        Swal.fire({
            title: "Успешно",
            text: "Данные листа оценки состояния пациента успешно изменены!",
            icon: "success",
            timer: 2500,
            timerProgressBar: true
        });
        ...
        Swal.fire({
            title: "Ошибка",
            text: "Лист оценки состояния пациента не изменен!",
            icon: "error",
            timer: 2500,
            timerProgressBar: true
        });
        ...
        <BCol class="col-xl-12">
            <BCard no-body>
                ...
                <div class="d-flex justify-content-end align-items-center gap-3 p-3">
                    ...
                    <button class="btn btn-primary" @click="onHandlesubmit">Изменить</button>
                </div>
