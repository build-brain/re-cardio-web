...
const displayWeight = computed(() => healthDATA.value.weight ?? ECR.value.weight);
const displayBodyMassIndex = computed(() => healthDATA.value.body_mass_index ?? ECR.value.body_mass_index);
const displaySystolicPressure = computed(() => healthDATA.value.systolic_pressure ?? ECR.value.systolic_pressure);
const displayDiastolicPressure = computed(() => healthDATA.value.diastolic_pressure ?? ECR.value.diastolic_pressure);
const displayPulseRate = computed(() => healthDATA.value.pulse_rate ?? ECR.value.pulse_rate);
const displayRespiratoryRate = computed(() => healthDATA.value.respiratory_rate ?? ECR.value.respiratory_rate);
const displayBodyTemperature = computed(() => healthDATA.value.body_temperature ?? ECR.value.body_temperature);
const displaySpo2 = computed(() => healthDATA.value.blood_saturation ?? ECR.value.spo2);
const displayAnxietyLevel = computed(() => healthDATA.value.anxiety_level ?? ECR.value.anxiety_level);
const displayDepressionLevel = computed(() => healthDATA.value.depression_level ?? ECR.value.depression_level);
        ...
        <BCol lg="4">
            ...
            <BCard>
                <BCardHeader class="d-flex justify-content-between p-0 mb-2">
                    ...
                    <router-link :to='`/ecr/add-admission-data/${route.params.id}`' v-if="admission_data === null">
                    ...
                    <router-link :to='`/ecr/edit-admission-data/${route.params.id}`' v-if="admission_data !== null">
                        <button type="button" class="btn bg-primary btn-sm ms-auto bg-opacity-10">
      ...
      <BRow class="mb-3">
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-ruler-line"></i>&ensp;{{ $t('health_data.height') }}</strong>
            <h1 class="text-success">{{ displayHeight }}</h1>
          </div>
        </BCol>
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-scales-2-fill"></i>&ensp;{{ $t('health_data.weight') }}</strong>
            <h1 class="text-success">{{ displayWeight }}</h1>
          </div>
        </BCol>
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-scales-2-fill"></i>&ensp;{{ $t('health_data.pulse') }}</strong>
            <h1 class="text-success">{{ displayBodyMassIndex }}</h1>
          </div>
        </BCol>
      </BRow>
      <BRow class="mb-3">
        <BCol cols="12" sm="4">
          <div class="d-flex w-100">
            <div class="d-flex flex-column">
              <strong class="text-primary size-profile"><i class="ri-stethoscope-line"></i>&ensp;{{ $t('health_data.chss') }}</strong>
              <div class="d-flex justify-content-evenly">
                <h1 class="text-success">{{ displaySystolicPressure }}</h1>
                <h6 class="text-success">/</h6>
                <h1 class="text-success">{{ displayDiastolicPressure }}</h1>
              </div>
            </div>
          </div>
        </BCol>
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-heart-pulse-fill"></i>&ensp;ЧСС, уд/мин</strong>
            <h1 class="text-success">{{ displayPulseRate }}</h1>
          </div>
        </BCol>
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-windy-fill"></i>&ensp;ЧДД, в.в./мин</strong>
            <h1 class="text-success">{{ displayRespiratoryRate }}</h1>
          </div>
        </BCol>
      </BRow>
      <BRow class="mb-3">
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-temp-cold-fill"></i>&ensp;{{ $t('health_data.temperature') }}</strong>
            <h1 class="text-success">{{ displayBodyTemperature }}</h1>
          </div>
        </BCol>
        <BCol cols="12" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile">
              <img width="15" height="15" src="/oxygen-svgrepo-com.svg">&ensp;{{ $t('health_data.spo2') }}
            </strong>
            <h1 class="text-success"> {{ displaySpo2 }} %</h1>
          </div>
        </BCol>
        <BCol cols="6" sm="4">
          <div class="d-flex flex-column">
            <strong class="text-primary size-profile"><i class="ri-emotion-sad-fill"></i>&ensp;HADS</strong>
            <h1 class="text-success">{{ displayAnxietyLevel }}/{{ displayDepressionLevel }}</h1>
          </div>
        </BCol>
      </BRow>
    </div>
        ...
        <h5 class="text-muted">Нет данных</h5>
            ...
            <BCard>
                <BCardHeader class="d-flex justify-content-between p-0 mb-2">
                    ...
                    <router-link :to='`/ecr/profile-erc/${route.params.id}/activities`'>
                        <button type="button" class="btn bg-primary btn-sm ms-auto bg-opacity-10">
                            <i class="ri-eye-fill text-primary align-bottom me-1"></i>
                            <span class="text-primary fw-bold text-opacity-100">Просмотр</span>
                        </button>
            ...
            <BCard>
                <BCardHeader class="d-flex justify-content-between p-0 mb-2">
                    ...
                    <router-link :to='`/ecr/profile-erc/${route.params.id}/event`'>
                        <button type="button" class="btn bg-primary btn-sm ms-auto bg-opacity-10">
                            <i class="ri-eye-fill text-primary align-bottom me-1"></i>
                            <span class="text-primary fw-bold text-opacity-100">Просмотр</span>
                        </button>
                    ...
                    <BCard no-body>
                        <BCardBody class="card-height2">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <router-link :to="`/ecr/add-file/${$route.params.id}/`">
