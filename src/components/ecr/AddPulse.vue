<template>
    <div>
            ...
            <BCard no-body id="applicationList">
                <BCardHeader class="border-0">
                    <div class="d-md-flex align-items-center">
                        <h5 class="card-title mb-3 mb-md-0 flex-grow-1">ЧСС</h5>
                            ...
                            <table class="table table-nowrap align-middle" id="jobListTable">
                                <thead class="text-muted table-light">
                                    <tr class="text-uppercase">
                                        ...
                                        <th class="small-font sort" @click="sortBy('creation_date', 'pulse_rate')">
                                            Дата
                                        </th>
                                        <th class="small-font sort" @click="sortBy('pulse_rate', 'pulse_rate')">
                                            Пульс
                                        </th>
                                        <th class="small-font sort" @click="sortBy('pulse_on_exertion', 'pulse_rate')">
                                            Тип Пульса
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="list form-check-all">
                                    <tr v-for="(item, index) in filteredTempList" :key="index">
                                        ...
                                        <td class="designation">
                                            {{ 
                            item.pulse_on_exertion
        ? (item.pulse_on_exertion === 'recommended_limits' ? 'в рекомендуемых пределах'
            : item.pulse_on_exertion === 'brief_exceeding' ? 'кратковременное превышение'
            : item.pulse_on_exertion === 'prolonged_exceeding' ? 'длительное превышение'
            : 'не указано')
        : 'не указано'
}}
                                        </td>
        ...
        <BModal v-model="createAppModal" id="showModal" title="Пульс" header-class="bg-light p-3"
                ...
                <BCardBody class="border  border-0">
                    <BForm @submit.prevent="addTemp">
                        <BRow class="g-3">
                            <BCol sm="12">
                                <div class="mb-3">
                                    <label class="form-label">ЧСС</label>
                                    <input v-model="form_temp.pulse_rate" type="number" placeholder="уд/мин" :class="{'is-invalid': v$.pulse_rate.$error }"
                                        class="form-control" />
                                        <span v-if="v$.pulse_rate.$error" class="text-danger">Это обязательное поле</span>

                                </div>
                                <div class="my-3">
                                    <label class="form-label">Тип ЧСС </label>
                                    <select class="form-select" v-model="form_temp.pulse_on_exertion"  :class="{'is-invalid': v$.pulse_on_exertion.$error }">
                                        <option disabled selected :value="null">Выбрать...</option>
                                        <option v-for="(translation, key) in pulseTypeTranslations" :key="key" :value="key">{{ translation }}</option>
                                    </select>
                                    <span v-if="v$.pulse_on_exertion.$error" class="text-danger">Выберите тип!</span>
                                ...
                                <div class="my-3">
                                    ...
                                    <flat-pickr placeholder="Выберите дату" v-model="form_temp.creation_date"
                                        ...
                                        <span v-if="v$.creation_date.$error" class="text-danger">Выберите тип!</span>
...
const temp_list = computed(() => {
    return store.state.charts.pulse_rate.filter(item => item.pulse_rate !== null && item.pulse_on_exertion !== null);
});
...
const filteredTempList = computed(() => {
    ...
    if (searchTermTrimmed !== '') {
        return tempList.filter(item => {
            const dyspneaMatches =  (item.pulse_rate !== null && item.pulse_rate.toString().includes(searchTerm.value.trim()));
            const creationDateMatches = item.creation_date !== null && item.creation_date.toString().toLowerCase().includes(searchTermTrimmed);
            const dyspneaTypeMatches = item.pulse_on_exertion !== null && pulseTypeTranslations[item.pulse_on_exertion].toLowerCase().includes(searchTermTrimmed);
...
const addTemp = async () => {
    ...
    try {
        const response = await axiosInstance.post(`/health-diary-records/add_pulse_rate/?by_er_card=${route.params.id}`, form_temp.value);
    ...
    creation_date: new Date().toISOString().split('T')[0] 
            ...
            temp_list.value = store.state.charts.pulse_rate;
        }
    } catch (error) {
        Swal.fire({
            icon: 'error',
